...
class transformer(nn.Module):
    def __init__(self, d_shape:int, n_layers:int=2, word_window:int=512):
        ...
        # Output Layers
        self.w = nn.Linear( d_shape * word_window, 2)
    ...
    def forward(self, in_emb, out_emb ):
        ...
        #return self.w(torch.flatten(d_output, start_dim=1))
        #return F.sigmoid( self.w(torch.flatten(d_output, start_dim=1)) )
        #return F.relu( self.w(torch.flatten(d_output, start_dim=1)) )
